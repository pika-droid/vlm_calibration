"""
Centralized configuration for the Matryoshka Multimodal Model (M3-LLaVA) Calibration evaluation pipeline.
...
@dataclass
class EvalConfig:
    """Configuration for the multi-scale VLM calibration evaluation of M3-LLaVA."""

    # ── Model ────────────────────────────────────────────────────────────
    model_path: str = "mucai/llava-v1.5-7b-m3"
    model_name: str = "llava-m3"
    precision: str = "fp16"  # "fp16" | "bf16" | "fp32"

    # ── Token Sweep ──────────────────────────────────────────────────────
    # M3-LLaVA supports nested visual token grids via progressive pooling:
    # 576 (no pooling), 144 (2x2), 36 (4x4), 9 (8x8), 1 (24x24).
    token_sweep: list[int] = field(
        default_factory=lambda: [1, 9, 36, 144, 576]
    )
    ...
    # ── Output ───────────────────────────────────────────────────────────
    output_dir: str = "/workspace/vlm-calibration-m3/results"
    plots_dir: str = "/workspace/vlm-calibration-m3/plots"
    logs_dir: str = "/workspace/vlm-calibration-m3/logs"
    checkpoint_dir: str = "/workspace/vlm-calibration-m3/checkpoints"
    ...
    def __post_init__(self) -> None:
        ...
        if not workspace_path.exists():
            ...
            if self.output_dir == "/workspace/vlm-calibration-m3/results":
                self.output_dir = str(project_root / "results_m3")
            if self.plots_dir == "/workspace/vlm-calibration-m3/plots":
                self.plots_dir = str(project_root / "plots_m3")
            if self.logs_dir == "/workspace/vlm-calibration-m3/logs":
                self.logs_dir = str(project_root / "logs_m3")
            if self.checkpoint_dir == "/workspace/vlm-calibration-m3/checkpoints":
                self.checkpoint_dir = str(project_root / "checkpoints_m3")
